...
function Sortable(el, opts) {
  ...
  if (!el)
    throw new TypeError('sortable(): expects an element');
  opts = opts || {};
...
Sortable.prototype.ontouchstart = function(e) {
  if (this.dragEl != null)
    return;
  if (this.ignored && closest(e.target, this.ignored, this.el))
    return;
  ...
  this.timer = setTimeout(function() {
    ...
    this.mouseStart = {};
    this.x = this.mouseStart.x = touch.clientX, this.y = this.mouseStart.y = touch.clientY;
...
Sortable.prototype.remove = Sortable.prototype.unbind = function() {
  this.events.unbind();
...
var positionHolder = function(touchDir) {
  ...
  while (el) {
    ...
    if (horizon) {
      ...
      if (touchDir === 1 && dx < -delta)
        break;
      if (touchDir === 3 && dx > delta)
        break;
      if (el === last) {
      ...
      if (touchDir === 2 && dy > delta)
        break;
      if (touchDir === 0 && dy < -delta)
        break;
      if (el === last) {
...
Sortable.prototype.reset = function() {
  ...
  if (handled)
    return setTimeout(cb.bind(this), 500);
